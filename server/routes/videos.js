...
// Get homePage videos
router.get("/home/videos/:number",verifyAccessToken, async(req, res) => {
    try {
        let num = req.params.number >= 8 ? 8 : 4; 
        const videos =  await Video.find({ visibility: {$ne: 2}},{videoPath : 0,watchtime : 0, description : 0,category : 0,comments : 0,likes : 0,dislikes : 0,isNFT : 0, nftOwners : 0}).limit(num).populate('creator','userId username ProfileAvatar isVerified isOnline followers')
                                   .sort({ views: -1 })
        res.send(videos)
...
router.get("/all/:skip/:limit",verifyAccessToken, async(req, res) => {
    try {
        ...
        const videos =  await Video.find({ visibility: {$ne: 2}},{videoPath : 0,watchtime : 0, description : 0,comments : 0,likes : 0,dislikes : 0,isNFT : 0, nftOwners : 0}).skip(skip * limit).limit(limit).populate('creator','userId username ProfileAvatar isVerified isOnline followers').exec()
        res.send(videos)
