...
nms.on("donePublish", async (id, StreamPath, args) => {
  ...
  try {
    ...
    var content = [];
    const livechat = await StreamChat.find({ liveId: live._id })
      .then((data) => {
    ...
    const saved = await new SavedStreamChat({
      streamId: ID,
      content: content,
    }).save();
    ...
    await ffmpeg.ffprobe(
      ...
      async (error, metadata) => {
        ...
        const live = await SavedLive.findOneAndUpdate(
          { streamId: ID },
          { duration: duration }
        );
      }
...
cron
  .schedule("* * 12 * *", async () => {
    const liveChat = await StreamChat.deleteMany({
...
cron
  .schedule("* * 24 * *", async () => {
    try {
      const savedLivesIpfsStore = await SavedLive.find({
        ipfsUrl: { $exists: false },
      }).then((data) => {
        if (data?.length > 0) {
          data?.map(async (item) => {
